...
def analyse_symbol(symbol: str, sector: str, ephe_path: str, dt: datetime, nak_method: str, manual_nak: str = None) -> SBCResult:
    ...
    # Separate hitting planets dynamically based on their energetic influence
    malefic_hitting_planets = []
    benefic_hitting_planets = []

    for pr in planet_results:
        if pr.is_vedha_hit:
            # Check if this planet is malefic under current temporal rules
            is_m = pr.planet in ["Sun", "Mars", "Saturn", "Rahu", "Ketu"] or (pr.planet == "Moon" and moon_malefic_paksha)
            if is_m:
                malefic_hitting_planets.append(pr.planet)
            else:
                benefic_hitting_planets.append(pr.planet)

    # Fallback to defaults if no specific planets are intersecting today
    res_planets_r2 = [malefic_hitting_planets[0]] if len(malefic_hitting_planets) > 0 else ["Saturn"]
    res_planets_r1 = [malefic_hitting_planets[1]] if len(malefic_hitting_planets) > 1 else ["Mars"] if len(malefic_hitting_planets) > 0 else ["Sun"]
    
    sup_planets_s1 = [benefic_hitting_planets[0]] if len(benefic_hitting_planets) > 0 else ["Jupiter"]
    sup_planets_s2 = [benefic_hitting_planets[1]] if len(benefic_hitting_planets) > 1 else ["Venus"] if len(benefic_hitting_planets) > 0 else ["Moon"]

    # Calculate dynamic volatility multipliers based on the total active matrix tension
    tension_multiplier = max(0.5, min(2.0, (bullish_count + bearish_count) / 4.0))

    r2_pct = 0.01 + (0.005 * tension_multiplier)  # Scales dynamically based on transit volume
    r1_pct = 0.004 + (0.004 * tension_multiplier)
    s1_pct = 0.004 + (0.004 * tension_multiplier)
    s2_pct = 0.01 + (0.008 * tension_multiplier)

    price_levels = [
        {
            "type": "resistance", 
            "price": cmp * (1.0 + r2_pct), 
            "label": "R2 Structural Matrix Peak", 
            "strength": "strong" if len(malefic_hitting_planets) > 2 else "moderate", 
            "planets": res_planets_r2, 
            "note": f"Upper target anchored by active malefic aspect rays. Volatility adjustment: +{r2_pct*100:.2f}%."
        },
        {
            "type": "resistance", 
            "price": cmp * (1.0 + r1_pct), 
            "label": "R1 Concentric Layer Target", 
            "strength": "moderate" if len(malefic_hitting_planets) > 0 else "weak", 
            "planets": res_planets_r1, 
            "note": "Immediate overhead resistance ceiling generated by primary horizontal intersection paths."
        },
        {
            "type": "pivot", 
            "price": cmp, 
            ...
            "strength": "neutral", 
            "planets": [], 
            "note": "Current base geometric equilibrium layout point."
        },
        {
            "type": "support", 
            "price": cmp * (1.0 - s1_pct), 
            "label": "S1 Concentric Layer Floor", 
            "strength": "moderate" if len(benefic_hitting_planets) > 0 else "weak", 
            "planets": sup_planets_s1, 
            "note": "Immediate tactical support zone constructed from active inner core timing layer values."
        },
        {
            "type": "support", 
            "price": cmp * (1.0 - s2_pct), 
            "label": "S2 Classical Panchaka Floor", 
            "strength": "strong" if len(benefic_hitting_planets) > 1 else "moderate", 
            "planets": sup_planets_s2, 
            "note": f"Major baseline demand channel where key benefic aspects converge. Volatility adjustment: -{s2_pct*100:.2f}%."
        }
    ]
