...
def listing_date_to_nakshatra(
    date_str: str, ephe_path: Optional[str] = None
) -> tuple[int, int, str]:
    """
    ...
    try:
        ...
        try:
            ...
            return (
                idx,
                pada,
                f"Sun nakshatra on listing date {date_str} (Moon unavailable)",
            )
        except:
            pass
...
for _i, _n in enumerate(
    ["Pushya", "Punarvasu", "Ardra", "Mrigashira", "Rohini", "Krittika", "Bharani"]
):
...
def get_active_directions(
    planet_name: str, speed: float, days_since_direct: Optional[float]
) -> list:
...
def vedha_strength(
    planet_name: str, planet_lon: float, sun_lon: float, moon_intra: float, mutual: bool
) -> str:
    if mutual:
        return "strong"
    base = planet_name.split()[0]
    if base == "Moon" and (moon_intra < 8.0 or moon_intra > 22.0):
        return "weak"
...
def is_benefic(
    planet_name: str, moon_lon: float, sun_lon: float, planet_lon: float, all_lons: dict
) -> bool:
    base = planet_name.split()[0]
    if base == "Moon":
        ...
        for on, ol in all_lons.items():
            if on.split()[0] in NATURAL_MALEFICS:
                if lon_to_nak_idx(ol) == mn and lon_to_pada(ol) == mp:
                    return False
        return True
    if base == "Mercury":
...
def compute_price_levels(
    ...
    def planets_near_nak(target_nak: int) -> list[str]:
        return [pr.planet for pr in planet_results if pr.planet_nak_idx == target_nak]
    ...
    planets_hitting_front = [
        pr.planet
        for pr in planet_results
        if pr.planet_nak_idx == front_idx
        or any(h in ["front", "front_vedha"] for h in pr.hits)
    ]
    levels.append(
        {
            "price": price_front,
    ...
    planets_hitting_left = [
        pr.planet
        for pr in planet_results
        if pr.planet_nak_idx == left_idx
        or any(h in ["left", "left_vedha"] for h in pr.hits)
    ]
    levels.append(
        {
            "price": price_left,
    ...
    planets_hitting_right = [
        pr.planet
        for pr in planet_results
        if pr.planet_nak_idx == right_idx
        or any(h in ["right", "right_vedha"] for h in pr.hits)
    ]
    levels.append(
        {
            "price": price_right,
...
def analyse_sbc(
    ...
    if stock_lon is not None:
        ...
        if date_str:
            stock_nak_idx, stock_pada, method_note = listing_date_to_nakshatra(
                date_str, ephe_path
            )
            stock_nak = NAKSHATRAS[stock_nak_idx] if stock_nak_idx < 27 else "Abhijit"
        else:
    ...
    def mutual_vedha(pa: PlanetState, pb: PlanetState) -> bool:
        ...
        return (planet_nak_indices[pb.name] in pa_t) and (
            planet_nak_indices[pa.name] in pb_t
        )
    ...
    for p in planets:
        ...
        hits = []
        if stock_nak_idx in activated:
            hits.append("direct")
        for d in active_dirs:
            t = nv.get(d)
            if t == stock_nak_idx and d not in hits:
                hits.append(d)
        ...
        strength = (
            "weak"
            if sun_wk
            else vedha_strength(p.name, p.lon, sun_lon, moon_intra, is_m)
        )
        ...
        if hits:
            ...
            if any(d in ["front", "direct", "front_vedha"] for d in hits):
                raw *= 1.5
                notes.append("FRONT/DIRECT hit → 1.5×")
            if is_m:
                b2 = 12.0 if not bene else -12.0
                raw += b2
                notes.append(f"Mutual Vedha → {b2:+.0f}")
            if cr:
                cb = 10.0 if not bene else -10.0
                raw += cb
                notes.append(f"Commodity match → {cb:+.0f}")
        ...
        planet_results.append(
            PlanetVedhaResult(
                ...
                is_combust=comb,
                mutual_vedha=is_m,
                commodity_relevant=cr,
                matched_commodities=cm,
                raw_score=raw,
                notes=notes,
            )
        )
    ...
    if sbc_score >= 72:
        sbc_label, sbc_color = "Strongly Bullish", "#00ff88"
    elif sbc_score >= 58:
        sbc_label, sbc_color = "Bullish", "#10b981"
    elif sbc_score >= 42:
        sbc_label, sbc_color = "Neutral", "#f59e0b"
    elif sbc_score >= 28:
        sbc_label, sbc_color = "Bearish", "#ef4444"
    else:
        sbc_label, sbc_color = "Strongly Bearish", "#991b1b"
...
def fetch_planet_states(
    ...
    # Ensure SWIEPH, SPEED, and SIDEREAL flags are set bitwise
    FLAGS = swe.FLG_SWIEPH | swe.FLG_SPEED | swe.FLG_SIDEREAL

    if dt is None:
        dt = datetime.now(timezone.utc)

    # Julian Day for target time
    jd = swe.julday(
        dt.year, dt.month, dt.day, dt.hour + dt.minute / 60.0 + dt.second / 3600.0
    )

    # Julian Day exactly 1 hour later to calculate accurate velocity manual steps
    jd_next = jd + (1.0 / 24.0)
    ...
    for name, pid in planet_ids.items():
        # Step A: Get current position data
        res, _ = swe.calc_ut(jd, pid, FLAGS)
        lon_now = res[0]
        speed_per_day = res[3]

        # Step B: Fallback calculation if velocity reads exactly 0 on future date steps
        if abs(speed_per_day) < 1e-5 and name != "Sun":
            res_next, _ = swe.calc_ut(jd_next, pid, FLAGS)
            lon_next = res_next[0]
            diff = (lon_next - lon_now) % 360
            if diff > 180:
                diff -= 360
            speed_per_day = diff * 24.0

        # Step C: ACCESS AND USE retro_history_days TO CALCULATE days_since_direct
        days_since_direct = None
        if speed_per_day > 0 and name in [
            "Mars",
            "Mercury",
            "Jupiter",
            "Saturn",
            ...
            # Scan backward day-by-day up to the retro_history_days limit
            for day_offset in range(1, retro_history_days + 1):
                past_dt = dt - timedelta(days=day_offset)
                past_jd = swe.julday(
                    past_dt.year,
                    past_dt.month,
                    past_dt.day,
                    past_dt.hour + past_dt.minute / 60.0,
                )
                past_res, _ = swe.calc_ut(past_jd, pid, FLAGS)
                past_speed = past_res[3]

                # If the planet was retrograde (< 0) in this window,
                # we have found when it turned direct!
                if past_speed < 0:
                    days_since_direct = float(day_offset)
                    break
    ...
    # Add Ketu exactly 180 degrees opposite to Rahu
    rahu = next(p for p in states if p.name == "Rahu")
    ketu_lon = (rahu.lon + 180.0) % 360
    states.append(
        PlanetState(name="Ketu", lon=ketu_lon, speed=rahu.speed, days_since_direct=None)
    )
