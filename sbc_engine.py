...
_NAK_ALIASES = {
    ...
    "ashlesha": 8, "ashl": 8,
    "magha": 9, "magh": 9,
    "purva phalguni": 10, "p. phalguni": 10, "p.phalguni": 10, "purva falguni": 10, "p.pha": 10,
    "uttara phalguni": 11, "u. phalguni": 11, "u.phalguni": 11, "uttra falguni": 11, "uttara falguni": 11, "u.pha": 11,
    "hasta": 12, "hast": 12,
    "chitra": 13, "chit": 13,
    "swati": 14, "swat": 14,
    "vishakha": 15, "visaka": 15, "vishaka": 15, "vish": 15, "visahka": 15,
    "anuradha": 16, "anu": 16,
    "jyeshtha": 17, "jyestha": 17, "jyes": 17,
    "moola": 18, "mool": 18, "mula": 18,
    "purva ashadha": 19, "purva shada": 19, "p. shada": 19, "p.sadha": 19, "p.asha": 19, "purva sadha": 19,
    "purva shada": 19, "p.shada": 19,
    "uttara ashadha": 20, "uttra shada": 20, "u. shada": 20, "u.sadha": 20, "u.asha": 20, "uttara sadha": 20,
    "uttara shada": 20, "uttra sadha": 20, "u.shada": 20, "uttara shada": 20,
    "shravana": 21, "shravan": 21, "srav": 21,
    "dhanishtha": 22, "dhanistha": 22, "dhan": 22,
    "shatabhisha": 23, "shatbhisha": 23, "satabhisha": 23, "shata": 23, "shatabhisaj": 23,
    "purva bhadrapada": 24, "purva bhadrapad": 24, "p. bhadrapada": 24, "p.bha": 24,
    "uttara bhadrapada": 25, "uttara bhadrapad": 25, "u. bhadrapada": 25, "u.bha": 25, "uttra bhadrapad": 25,
    "revati": 26, "reva": 26,
...
_VEDHA_RAW = {
    ...
    "Bharani":          {"front": "Magha",          "left": "Shravan",           "right": "Dhanishtha"}, # note: Shravan=Shravana
    "Krittika":         {"front": "Shravan",        "left": "Visaka",            "right": "Anuradha"},   # Shravan row 3 in img2
    "Rohini":           {"front": "Magha",          "left": "Abhijit",           "right": "Purva Bhadrapada"}, # corrected from table
    "Mrigashira":       {"front": "Shravan",        "left": "Vishakha",          "right": "Anuradha"},
    "Ardra":            {"front": "Magha",          "left": "Shravan",           "right": "Dhanishtha"},  # placeholder; see full table
    "Punarvasu":        {"front": "Uttara Ashadha", "left": "Uttara Phalguni",   "right": "Uttara Bhadrapada"},
    "Pushya":           {"front": "Purva Ashadha",  "left": "Uttara Falguni",    "right": "Uttara Shada"},
...
RETRO_GRACE_DAYS = {
    "Mars": 4, "Mercury": 3, "Jupiter": 8, "Saturn": 20, "Venus": 5
}
...
NAKSHATRA_COMMODITIES = {
    ...
    2:  ["rice", "oats", "metals", "til", "gems", "diamonds", "grams",
         "oils", "gold", "silver"],                                           # Krittika
    ...
    7:  ["silver", "gold", "ghee", "rice", "sambhar salt", "heeng",
         "sarso", "oil"],                                                     # Pushya
    8:  ["gur", "khand", "sonth", "masoor", "wheat", "chillies", "rice"],    # Ashlesha
    9:  ["oil", "til", "ghee", "moong", "gram", "gur", "alsi"],              # Magha
    10: ["woolen clothes", "blankets", "wool", "til", "oil", "silver"],      # Purva Phalguni
...
# Score weights
SCORE_WEIGHTS = {
    # hit × benefic_nature × strength
    ("hit", "benefic", "strong"):  -4.0,   # benefic hitting stock = bearish (book rule)
    ("hit", "benefic", "normal"):  -2.5,
    ("hit", "benefic", "weak"):    -1.0,
    ("hit", "malefic", "strong"):  +4.0,   # malefic hitting stock = bullish (book rule)
    ("hit", "malefic", "normal"):  +2.5,
    ("hit", "malefic", "weak"):    +1.0,
    # Sun special — weak vedha on others does not directly score on stock
    # but when Sun is the Vedha planet hitting the stock nakshatra:
    ("hit", "sun_malefic", "normal"):  +3.0,
    ("hit", "sun_malefic", "weak"):    +1.5,
    # Commodity relevance bonus
    "commodity_bonus": 1.5,
}

def _score_key(is_benefic_flag: bool, planet_name: str, strength: str) -> tuple:
    base = planet_name.split()[0]
    if base == "Sun":
        nature = "sun_malefic"
    elif is_benefic_flag:
        nature = "benefic"
    else:
        nature = "malefic"
    return ("hit", nature, strength)

def _compute_sun_weak_vedha_penalty(
    planet_name: str,
    planet_nak_idx: int,
    stock_vedha_idx_set: set[int],
    sun_nak_idx: int,
) -> float:
    """
    Book rule: Moon/Mars/Mercury/Jupiter/Venus/Saturn if Vedha'd by Sun = weak Vedha.
    This is already handled in vedha_strength() — this function is a placeholder
    for any additional penalty if needed.
    """
    return 0.0
...
def analyse_sbc(
    ...
    sector: str = "Unknown",
    stock_lon: Optional[float] = None,       # exact longitude if known
    ...
    Parameters
    ----------
    symbol      : stock ticker or name (used to derive natal nakshatra via hash)
    planets     : list of PlanetState for all 9 grahas at current transit
    sector      : yfinance sector string (e.g. "Financial Services")
    stock_lon   : if known, used to derive exact pada; else hash is used
    analysis_datetime : UTC datetime of analysis (defaults to now)
    """
    ...
    vedha_idx_set = {front_idx, left_idx, right_idx, stock_nak_idx}
    ...
    # ── Sun weak-vedha check: collect planets Sun is doing Vedha on ───────
    # (used in vedha_strength later)
    ...
    if sun_state:
        ...
        for d in sun_dirs:
            target = sun_nak_dirs.get(d, -1) if d != "self" else sun_nak_idx
            sun_hits.add(target)
        sun_hits.add(sun_nak_idx)   # Sun always influences its own nakshatra

    # ── First pass: compute which nakshatras each planet activates ────────
    planet_nak_indices: dict[str, int] = {}
    for p in planets:
        planet_nak_indices[p.name] = lon_to_nak_idx(p.lon)

    # ── Mutual Vedha detection ────────────────────────────────────────────
    # Two planets do mutual Vedha when each planet's activated direction
    # lands on the OTHER planet's nakshatra.
    ...
    for p in planets:
        ...
        # Which SBC directions does this planet activate, and do they hit the stock?
        nak_vedha = get_vedha_directions(nak_idx)
        activated_naks = {nak_idx}   # planet always influences its own nakshatra
        for d in active_dirs:
            if d in nak_vedha:
                activated_naks.add(nak_vedha[d])

        # Does any activated nakshatra = stock nakshatra or its vedha nakshatras?
        hits = []
        if stock_nak_idx in activated_naks:
            hits.append("direct")   # planet directly in stock nakshatra
        for d in active_dirs:
            target = nak_vedha.get(d)
            if target == stock_nak_idx:
                hits.append(d)
            # also check if planet nakshatra = one of stock's vedha nakshatras
            if nak_idx == front_idx and "front" in active_dirs:
                if "front" not in hits: hits.append("front_vedha")
            if nak_idx == left_idx and "left" in active_dirs:
                if "left" not in hits: hits.append("left_vedha")
            if nak_idx == right_idx and "right" in active_dirs:
                if "right" not in hits: hits.append("right_vedha")

        # Remove duplicates
        hits = list(dict.fromkeys(hits))

        # Sun weakens other planets — but Sun hitting stock directly still scores
        sun_weakened = (
            p.name != "Sun"
            and nak_idx in sun_hits
        )
        ...
        # Strength — sun_weakened overrides to weak
        if sun_weakened:
            strength = "weak"
        else:
            strength = vedha_strength(
                p.name, p.lon, sun_lon, moon_intra, is_mutual
            )
        ...
        notes = []
        if hits:
            key = _score_key(benefic_flag, p.name, strength)
            base_score = SCORE_WEIGHTS.get(key, 0.0)
            raw += base_score
            nature_str = "Benefic (Bearish)" if benefic_flag else "Malefic (Bullish)"
            notes.append(f"Vedha hit on stock nakshatra — {nature_str}, {strength} strength → {base_score:+.1f}")
            if comm_rel:
                raw += SCORE_WEIGHTS["commodity_bonus"]
                notes.append(f"Commodity relevance: {', '.join(comm_matches)} → +{SCORE_WEIGHTS['commodity_bonus']:.1f}")

        if is_mutual:
            notes.append("Mutual Vedha with another planet — strength elevated")

        if sun_weakened:
            notes.append("Sun is Vedha-ing this planet — Weak Vedha (reduced impact)")

        if p.days_since_direct is not None:
            base = p.name.split()[0]
            if base in RETRO_GRACE_DAYS and p.days_since_direct <= RETRO_GRACE_DAYS[base]:
                notes.append(f"Retrograde→Direct transition: {p.days_since_direct:.0f}d since direct (grace = {RETRO_GRACE_DAYS[base]}d)")

        debit = is_debilitated(p.name, p.lon)
        comb  = is_combust(p.name, p.lon, sun_lon)
        if debit:
            notes.append("Debilitated — Weak Vedha")
        if comb:
            notes.append("Combust — Weak Vedha")
    ...
    # ── Final score ───────────────────────────────────────────────────────
    # Normalize: total_raw range is roughly -30 to +30 → map to 0-100
    # Use tanh-based smooth mapping to avoid chronic extremes
    normalized = math.tanh(total_raw / 12.0)   # squash to -1..+1
    sbc_score = int(50 + normalized * 45)       # map to 5..95
    sbc_score = max(5, min(95, sbc_score))
...
def fetch_planet_states(
    ...
    for name, pid in planet_ids.items():
        ...
        states.append(PlanetState(
            name=name, lon=lon, speed=speed,
            days_since_direct=days_since_direct
        ))
...
def analyse_symbol(
    ...
    return analyse_sbc(symbol, planets, sector=sector,
...
def print_report(result: SBCResult) -> None:
    ...
    print(f"\n  Moon: Tithi {result.tithi} ({result.paksha} Paksha)"
          + ("  ⚠ Moon acting as MALEFIC (Paksha rule)" if result.moon_malefic_paksha else ""))
    ...
    print(f"\n{line}")
    print(f"  {'PLANET':<14}{'NAK':<22}{'DIRS':<14}{'HITS':<10}{'NATURE':<12}{'STRENGTH':<10}{'SCORE':>6}")
    print(line)
    ...
    for r in result.planet_results:
        ...
        flags = ""
        if r.is_debilitated: flags += "D"
        if r.is_combust:     flags += "C"
        if r.mutual_vedha:   flags += "M"
        score_str = f"{r.raw_score:+.1f}" if r.raw_score != 0 else "  0.0"
        nak_str = f"{r.planet_nak}" + (f"[{flags}]" if flags else "")
        print(f"  {r.planet:<14}{nak_str:<22}{dirs_str:<14}{hits_str:<10}{nature:<12}{r.strength:<10}{score_str:>6}")
        for note in r.notes:
            print(f"    {'↳'} {note}")
    ...
    print(f"  Bullish planets: {result.bullish_count}   "
